...
let words_starting_with_a = words_arr.filter(function word(word) {
...
function sq_of_arr_1(arr_elements) {
...
console.log(` `);
function checkscore(scores, max, min) {
    let numberexceedmax = scores.filter(checkmax).length;
    function checkmax(max_1) {
        return max_1 > max;
    }
    let numberfellbelow = scores.filter(checkmin).length;
    function checkmin(min_1) {
        return min_1 < min;
    }
    console.log(`The number of times the score exceeded the maximum score: ${numberexceedmax}`);
    console.log(`The number of times the score fell below the minimum score: ${numberfellbelow}`);
}
...
function odd_even_sum(array_11, input_1) {
    if (input_1 == "odd") {
        function check_odd(value) {
            return value % 2 !== 0;
        }
        let odd_numbers = array_11.filter(check_odd);
        let sum_odd = odd_numbers.reduce(function (a, b) { return a + b; }, 0);
        return sum_odd;
    }
    else if (input_1 == "even") {
        function check_even(value_1) {
            return value_1 % 2 === 0;
        }
        let even_numbers = array_11.filter(check_even);
        let sum_even = even_numbers.reduce(function (a, b) { return a + b; }, 0);
        return sum_even;
    }
}
...
function check_element(array_12, input_2) {
...
function find_smallest_number(array_13) {
    array_13.sort(function (a, b) { return a - b; });
    return array_13[0];
}
...
function filterbylength(array_1, n) {
...
let duplicates_1 = [];
function findDuplicates(array_2) {
...
function countOccurrences(array_9, n) {
    let count = 0;
...
let x_8;
function isSorted(array_8) {
    x_8 = array_8.slice(0, array_8.length);
    array_8.sort();
    for (let i = 0; i < array_8.length; i++) {
        if (x_8[i] == array_8[i] || x_8[i] == array_8.sort(function (a, b) { return a - b; })) {
            return true;
        }
        else {
            return false;
...
function arr_to_string(array_4) {
...
let x_5;
let x_6;
function swapElements(array_7, index1, index2) {
...
let count = 0;
function find_match(string_1, n) {
...
let a_1 = 6;
let b_2 = 7;
function operation(operand_1, operand_2, operation_1) {
    if (operation_1 === '+') {
